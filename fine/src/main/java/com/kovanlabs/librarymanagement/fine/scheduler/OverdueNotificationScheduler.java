...
public class OverdueNotificationScheduler {
    ...
    public void sendOverdueNotifications() {
        ...
        Map<Users, List<Borrow>> userBorrowsMap = overdueBorrows.stream()
                .filter(b -> b.getUsers() != null && b.getUsers().getEmail() != null)
                .collect(Collectors.groupingBy(Borrow::getUsers));

        for (Map.Entry<Users, List<Borrow>> entry : userBorrowsMap.entrySet()) {
            ...
            BigDecimal userTotalPendingFine = fineService.calculateTotalPendingFineForUser(user.getId());
