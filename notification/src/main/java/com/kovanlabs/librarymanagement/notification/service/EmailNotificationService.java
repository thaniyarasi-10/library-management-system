...
public class EmailNotificationService implements NotificationService {
    ...
    public void send(NotificationRequest request) {
        ...
        if (mailSender != null) {
            try {
                ...
                log.error("Failed to send HTML Email via JavaMailSender to {}: {}", request.recipient(), e.getMessage(), e);
            }
        } else {
    ...
    public String formatHtmlTemplate(NotificationRequest request) {
        ...
        if (overdueBooks != null && !overdueBooks.isEmpty()) {
            for (OverdueBookDto book : overdueBooks) {
                ...
                bookRowsHtml.append("<tr>")
                        ...
                        .append("<td>").append(escapeHtml(book.author() != null ? book.author() : "N/A")).append("</td>")
                        .append("<td>").append(book.dueDate() != null ? book.dueDate().toString() : "N/A").append("</td>")
        ...
        double finalFine = (request.totalFine() != null && request.totalFine() > 0) ? request.totalFine() : calculatedTotalFine;
